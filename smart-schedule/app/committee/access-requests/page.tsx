  ...
  const handleLock = async (requestId: string) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(response.error || 'Failed to lock request')
      }
    } catch (err) {
  ...
  const handleApprove = async () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (!token) {
        ...
        if (user) {
          toast.error('Your session token is missing. Please log out and log back in to refresh your session.')
          return
        }
      }
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.success('Request approved successfully! An account has been created and an email with login credentials has been sent to the requester.')
      } else {
        ...
        if ((errorMsg.includes('Authentication required') || errorMsg.includes('session expired')) && 
          ...
          toast.error('Your session has expired. Please log out and log back in to refresh your session.')
          // Clear auth state
          localStorage.removeItem('smartSchedule_user')
          localStorage.removeItem('smartSchedule_auth')
          localStorage.removeItem('smartSchedule_token')
      ...
      if ((errorMessage.includes('Authentication') || errorMessage.includes('session expired')) && 
        ...
        toast.error('Your session has expired or there was a connection error. Please log out and log back in.')
        localStorage.removeItem('smartSchedule_user')
        localStorage.removeItem('smartSchedule_auth')
        localStorage.removeItem('smartSchedule_token')
        globalThis.window.location.href = '/login'
      } else if (errorMessage.includes('Failed to fetch')) {
        toast.error('Connection error: Please check your network connection and ensure the backend server is running.')
      } else {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setDecisionNote('')
        await fetchRequests()
        toast.success('Request rejected. An email has been sent to the requester.')
      } else {
    ...
    return (
      <ProtectedRoute requiredRole="committee">
        <div className="min-h-screen bg-gray-50">
          <AppHeader title="Access Requests" showBack={true} />
          <div className="max-w-7xl mx-auto px-4 py-8">
            <p className="text-red-600">Access denied. Committee role required.</p>
          </div>
        </div>
      </ProtectedRoute>
    )
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="mb-6">
            ...
            <p className="text-gray-600 mt-2">Review and manage access requests from students and faculty</p>
          </div>
