...
export async function POST(
  ...
  try {
    ...
    const { universityId, scheduleId } = body

    if (!universityId || !scheduleId) {
      return NextResponse.json(
        { success: false, error: 'University ID and Schedule ID are required' },
        { status: 400 }
      )
    }
...
// DELETE /api/sections/[id]/enroll - Unenroll student from section
